  ...
  return (
    ...
    >
      ...
      <div
        ref={panelRef}
        tabIndex={-1}
        className={`glass relative w-full ${wide ? "max-w-3xl" : "max-w-md"} rounded-2xl border border-hairline-strong p-6 shadow-[var(--shadow-pop)] focus-ring`}
      >
        {title && <h2 className="mb-4 text-lg font-semibold">{title}</h2>}
        <button
          onClick={() => onCloseRef.current()}
          aria-label="Close"
          className="focus-ring absolute right-4 top-4 rounded-md p-1 text-muted hover:text-ink"
        >
          ✕
        </button>
        {children}
